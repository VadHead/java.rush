package javarush.lvl3.lec04;
/*taskKey="com.javarush.task.task03.task0309"
Сумма 5 чисел
Вывести на экран сумму чисел от 1 до 6 построчно (должно быть 6 строк):
1
1+2=3
1+2+3=6
...
Пример вывода:
1
3
6
...
Требования:
1.	Программа должна выводить текст.
2.	Выведенный текст должен содержать 10 строк.
3.	Число в каждой новой строке должно быть больше предыдущего.
4.	Выводимый текст должен соответствовать заданию.
*/
public class task05 {
    public static void main(String[] args) {
       System.out.println(1);
        System.out.println(1+2);
       System.out.println(1+2+3);//напишите тут ваш код
        System.out.println(1+2+3+4);
        System.out.println(1+2+3+4+5);

    }
}

